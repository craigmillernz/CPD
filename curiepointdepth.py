...
def splitgrid(working_dir, gridfilename, window_width, overlap_factor):
    ...
    working_dir is the main directory where the main data grid lives and is
    where processing subdirectories will be created.
    
    gridfilename is the main data grid to be split
    
    window_width is the window width in metres (ie 100 km = 100000)
    ...
    #main_dir = ('C:/Users/craigm/Dropbox/Yukon_CPD_project/Yukon/')
        ...
        if fn.endswith(".grd"):
            ...
            #plot using graphics.imshow library
                ...
                graphics.imshow_hs(mag,ax, cmap_norm='equalize', hs=True, colorbar=True,
                                   ...
                                   contours=False, labels=False, fmt='%.0f',
    ...
    graphics.imshow_hs(mag,ax, cmap_norm='equalize',hs=True,colorbar=True,
                       cb_ticks='stats',nSigma=2,azdeg=45,altdeg=45, 
                       blend_mode='alpha',alpha=0.7, extent=(xmin, xmax, ymin, ymax), 
                       origin='upper', contours=False, labels=False, fmt='%.0f', 
                       levels=np.arange(round(np.nanmin(mag),0),
                                        round(np.nanmax(mag),0), 2))
    
    ax.scatter(centerx, centery)
    
    utils.rectangles(centerx, centery, window_width, window_width,facecolor='None') 
...
def runfft(working_dir, window_dir):
    ...
    working_dir is the main directory where the main grid file is located
    
    window_dir is the name of the window_dir created when the grids were split
    
    Check the location of grdfft
...
def calccpd(working_dir, window_dir, Zo_start, Zo_end, Zt_start, Zt_end):
    '''
    Read in the output from the GMT grdfft.  Use the output in wavenumbers not wavelengths
    
    Zo_start, Zo_end, Zt_start, Zt_end are the wavenumber values used to fit
    the straight line to.  May take some experimenting to find appropriate values

    Loops through each file in directory and then incorporates the centre coords of each window 
    to the results, so then I can have a X,Y,Zbase,Ztop,Zcent file to grid. 
    ...
    It writes a file called "cpd_results.csv' in the results directory
    
    Spectra plots are written in the /spectra_plots directory
    '''
    plt.close('all')
    ...
    gridcenters = pd.read_csv(griddir + dsep + window_dir+'_km_center_coords.csv',
                              comment='#', names=['window','east','north'],
                              index_col='window')
    ...
    for fn in os.listdir(datadir):
        if fn.endswith(".txt"):
            ...
            #data = pd.read_csv(datadir + dsep + 'Window_' +str(window_num) + '.txt', sep='\t', names=['Wavenumber','Power','Stdev'], comment='#')
            data = pd.read_csv(datadir + dsep + fn, sep='\t', 
                               names=['Wavenumber','Power','Stdev'], comment='#')
            #%%
            #Calculate centroid depth, Zo
            #choose which points to fit
            
            #centroid fit points (k/2pi), may need to adjust slightly if  returns an error
            Zo_start=Zo_start #0.003
            Zo_end = Zo_end #0.036
            ...
            #fit the data
            centroid_fit_wavenumber = sm.add_constant(centroid_fit_wavenumber) #Use the add_constant to get sm to not go through 0.
            cent_model =sm.OLS(centroid_fit_power, centroid_fit_wavenumber)
            cent_results = cent_model.fit()
            #print cent_results.summary()
            
            #prstd, iv_l, iv_u = wls_prediction_std(cent_results)
            ...
            print ur"centroid depth = %0.1f \u00B1 %0.1f km" %(centroid_depth, centroid_depth_err)
            
            #%%
            #Calculate top depth Zt
                      
            Zt_start=Zt_start #0.05
            Zt_end = Zt_end  #0.14 
            ...
            #fit the data
            top_fit_wavenumber = sm.add_constant(top_fit_wavenumber) #Use the add_constant to get sm to not go through 0.
            top_model =sm.OLS(top_fit_power, top_fit_wavenumber)
            top_results = top_model.fit()
            #print top_results.summary()
            
            #prstd, iv_l, iv_u = wls_prediction_std(top_results)
            ...
            print ur'top depth = %0.1f \u00B1 %0.1f km' %(top_depth, top_depth_err)
            print ur'centroid depth = %0.1f \u00B1 %0.1f km' %(centroid_depth, centroid_depth_err)
            #%%
            ...
            #Plot raw spectrum
            plt.subplot(131)
            plt.scatter(data.Wavenumber*1000, data.Power, marker='o', s=2, label="Data")
            ...
            #plot centroid
            plt.subplot(132)
            plt.scatter(data.wavenumber_2pi, data.y_centroid, marker='o', s=2, label="Data")
            #plt.errorbar(data.wavenumber_2pi, data.y_centroid, yerr=data.y_centroid_std, marker='o', markersize=2, errorevery=10, label="Data")
            
            plt.plot(centroid_fit_wavenumber, cent_results.fittedvalues, 'r-', label="Linear fit")
            ...
            #plot top
            plt.subplot(133)
            plt.scatter(data.wavenumber_2pi, data.y_top, marker='o', s=2, label="Data")
            #plt.errorbar(data.wavenumber_2pi, data.y_top, yerr=data.y_top_std, marker='o', markersize=2, errorevery=10, label="Data")
            plt.plot(top_fit_wavenumber, top_results.fittedvalues, 'r-', label="Linear fit")
            plt.xlim(0, data.wavenumber_2pi.max())
            plt.xlabel(r'$|k|/(2 \pi)$ [km$^{-1}$]')
    ...
    results = pd.DataFrame(centroid_depth_table, index=window_table, columns=['centroid_depth_table'])
    ...
    result.to_csv(resultdir + dsep + 'cpd_results.csv', 
                  header=['window', 'east', 'north', 'centroid_depth', 'cent_depth_err', 'cent_rsq', 'top_depth', 'top_depth_err', 'top_rsq', 'base_depth', 'base_depth_err'],
                  columns=['window', 'east', 'north', 'centroid_depth_table', 'centroid_depth_err_table', 'centroid_rsq_table', 'top_depth_table', 'top_depth_err_table', 'top_rsq_table', 'base_depth_table', 'base_depth_err_table'],
